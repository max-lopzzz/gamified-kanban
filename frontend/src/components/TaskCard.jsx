...
export default function TaskCard({
  ...
  const [assignees, setAssignees] = useState(
    encodeAssignees(task.assignees || [])
  );

  // Subtasks are edited live; keep a local copy that resyncs on board reload.
  ...
  async function addSubtask(titleText) {
    try {
      const created = await api.createSubtask(task.id, titleText);
  ...
  const xp = Math.round(
    task.story_points * XP_PER_POINT * (PRIORITY_MULT[task.priority] ?? 1)
  );
  ...
  async function save(e) {
    ...
    try {
      ...
      return; // Board shows the error; keep the editor open
    }
    setEditing(false);
  }
    ...
    return (
      <div className="task-card task-card-editing">
        <form onSubmit={save}>
          ...
          <TokenMultiSelect
            options={dependencyOptions(allTasks, task.id)}
            value={dependencyIds}
